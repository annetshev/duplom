...
const ScienceOfficer = () => {
  ...
  const handleDeleteDocument = async (documentId) => {
    try {
      ...
      // Отримання оригінального об'єкту document за його id
      ...
      const documentType = documentToDelete.type;

      // Перевірка, чи дозволено видалення документа даного типу
      if (documentType === "НДР") {
        console.error("Deletion of 'НДР' documents is not allowed");
        return;
      }

      // Фільтрація масиву documents, щоб видалити об'єкт з відповідним id
      setDocuments(documents.filter((doc) => doc.id !== documentId));

      let apiURL;
      switch (documentType) {
        case "Наукова робота":
          apiURL = `${API_BASE_URL}/scientific_materials/${documentId}`;
          break;
        case "Рецензія":
        case "Тези":
          apiURL = `${API_BASE_URL}/reviews_abstracts/${documentId}`;
          break;
        default:
      ...
      // Відправлення запиту на видалення документа
  ...
  const handleEditDocument = (document) => {
    ...
    switch (document.type) {
      case "НДР":
        editUrl = `${API_BASE_URL}/ndr/${document.id}`;
        break;
      case "Наукова робота":
        editUrl = `${API_BASE_URL}/scientific_materials/${document.id}`;
        break;
      case "Рецензія":
        editUrl = `${API_BASE_URL}/reviews_abstracts/${document.id}`;
        break;
      case "Тези":
        editUrl = `${API_BASE_URL}/reviews_abstracts/${document.id}`;
        break;
